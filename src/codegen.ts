...
async function generateContractTypes() {
  ...
  execSync(`sed -e '/type\:\ \"receive\"/d' -i ${extraFlag} ${path.join(typesDir, "Timelock.ts")}`, {cwd: ROOT_DIR})
  execSync(`sed -e '/type\:\ \"receive\"/d' -i ${extraFlag} ${path.join(typesDir, "TimelockMock.ts")}`, {cwd: ROOT_DIR})
}
